...
const authenticateJWT = async (req, res, next) => {
  ...
  if (accessToken && refreshToken) {
    ...
    try {
      ...
      const foundItem = await db.Login.findOne({ where: { mode: 'user', user_id: user.id } });
      if (foundItem) {
    ...
    if (result.error) {
      return res.status(401).send({ message: 'Unauthorized3! ' + result.message });
    }
...
const reGenerateAccessToken = async (decryptRefreshToken, res) => {
  ...
  try {
    ...
    if (decode) {
      ...
      const foundItem = await db.Login.findOne({ where: { mode: 'user', user_id: decode.id } });
      if (foundItem) {
      ...
      // @ts-ignore
      let payload = await db.User.scope("withPublic").findByPk(decode.id);
      // console.log('payload', payload);
      let accessToken = null
      if (payload) {
        await db.User.update({ refreshAt: Date() }, { where: { id: decode.id } });
...
const setUpCookie = (res, accessToken = null, user = null) => {
  ...
  try {
    if (accessToken && !user) {
      ...
      res.cookie('accessToken', encryptToken, {
